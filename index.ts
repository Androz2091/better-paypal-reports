...
const argFileName = argv.file as string;
const argDirName = argv.dir as string;

if (!argFileName && !argDirName) {
...
const getFileName = async () => {
    ...
    if (argFileName) {
        ...
        let files = await promises.readdir(argDirName)
        return argDirName + '/' + (files.map((fileName) => {
            return {
                name: fileName,
                time: statSync(argDirName + '/' + fileName).mtime.getTime()
            };
        })
...
const showResult = (fileName: string) => {
...
getFileName().then((res) => showResult(res));
